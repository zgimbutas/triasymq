C       gaussq.f -- Gaussian quadrature nodes and weights by the
C       Golub-Welsch algorithm (Stanford, 1975; revised 1982).
C
C       Source: netlib.org/go/gaussq.f. The routine imtql2 is from
C       EISPACK. The authors assert no copyright and attach no license;
C       netlib states that its software has no restrictions on use
C       (netlib FAQ 2.3). This file is not covered by the COPYING terms
C       of this repository.
C
C       Local change: the function DGAMMA at the end of the file calls
C       the Fortran GAMMA intrinsic. The netlib copy expects dgamma from
C       fnlib, and the copy this was taken from stubbed it to return 1.
C
